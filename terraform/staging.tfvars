resource_group_name = "culinary-platform-staging"
# Назва плану служби додатків Azure (Azure App Service Plan)
azurerm_service_plan_name = "culinary-service-plan-staging"

# Назва Linux-додатку (WebApp) для бекенду
azurerm_linux_web_app_name_backend_name = "culinary-backend-app-staging"

# Назва Linux-додатку (WebApp) для фронтенду
azurerm_linux_web_app_name_frontend_name = "culinary-frontend-app-staging"

# Назва сервера PostgreSQL (PostgreSQL Flexible Server)
azurerm_postgresql_flexible_server_name = "culinary-db-staging"
